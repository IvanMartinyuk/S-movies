using DAL.Context;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Text;
using System.Text.Json;
using System.Threading.Tasks;

namespace DAL
{
    public class IMDbService
    {
        List<Actor> Actors { get; set; } = new List<Actor>();
        List<Genre> Genres { get; set; } = new List<Genre>();
        List<Producer> Producers { get; set; } = new List<Producer>();
        List<Director> Directors { get; set; } = new List<Director>();
        Film Film { get; set; } = new Film();
        FilmContext context { get; set; }
        string key = "36a979b383msh58c4184b01300a8p14d725jsn79815b6808db";
        HttpClient client = new HttpClient();
        public async Task FillData(FilmContext context, int max = 100)
        {
            this.context = context;
            if (!context.Companies.Any(x => x.Name == "none"))
            {
                context.Companies.Add(new Company() { Name = "none", Description = "none" });
                context.SaveChanges();
            }
            Company companynone = context.Companies.FirstOrDefault(x => x.Name == "none");
            int companyid = companynone.Id;
            using (var response = await SendRequest(new Uri("https://imdb8.p.rapidapi.com/title/get-most-popular-movies?homeCountry=UA&purchaseCountry=UA&currentCountry=UA")))
            {
                response.EnsureSuccessStatusCode();
                JArray mass = (JArray)JsonConvert.DeserializeObject(await response.Content.ReadAsStringAsync());
                for(int i = 0; i < mass.Count(); i++)
                {
                    if (i == max)
                        return;
                    string filmid = mass[i].ToString().Split(new char[] { '/' })[2];
                    if (!context.Films.Any(x => x.ImdbId == filmid))
                    {
                        Film.ImdbId = filmid;
                        try
                        {
                            SetGenresAndFilmData(filmid).Wait();
                            GetFilmDescription(filmid).Wait();
                            GetFullCredits(filmid).Wait();
                        }
                        catch (HttpRequestException ex)
                        {

                        }
                        Film.CompanyId = companyid;
                        context.Films.Add(Film);
                        context.SaveChanges();
                        Film = context.Films.FirstOrDefault(x => x.Title == Film.Title
                                                            && x.Image == Film.Image
                                                            && x.DateOfPublishing == Film.DateOfPublishing);
                        Film.Actors = Actors;
                        Film.Directors = Directors;
                        Film.Genres = Genres;
                        Film.Producers = Producers;
                        Film.Selections = new List<Selection>();
                        context.Films.Update(Film);
                        context.SaveChanges();
                        Clear();
                    }
                }
            }
        }
        async Task SetGenresAndFilmData(string id)
        {
            using (var response = await SendRequest(new Uri("https://imdb8.p.rapidapi.com/title/get-meta-data?ids=" + id + "&region=US")))
            {
                response.EnsureSuccessStatusCode();
                JObject obj = (JObject)JsonConvert.DeserializeObject(await response.Content.ReadAsStringAsync());
                Film.Title = obj[id]["title"]["title"].ToString();
                Film.ImdbRating = Convert.ToDouble(obj[id]["ratings"]["rating"]);
                Film.LocalRating = 10;
                Film.DateOfPublishing = Convert.ToDateTime(obj[id]["releaseDate"]);
                Film.Image = obj[id]["title"]["image"]["url"].ToString();
                foreach (string name in obj[id]["genres"])
                {
                    if (!context.Genres.Any(x => x.Name.ToLower().Equals(name.ToLower() )))
                        Genres.Add(new Genre() { Name = name });
                    else
                        Genres.Add(context.Genres.FirstOrDefault(x => x.Name.ToLower().Equals(name.ToLower())));
                }
            }
        }
        async Task GetFilmDescription(string id)
        {
            using (var response = await SendRequest(new Uri("https://imdb8.p.rapidapi.com/title/get-plots?tconst=" + id)))
            {
                response.EnsureSuccessStatusCode();
                JObject obj = (JObject)JsonConvert.DeserializeObject(await response.Content.ReadAsStringAsync());
                Film.Description = obj["plots"][0]["text"].ToString();
            }
        }
        async Task GetFullCredits(string id)
        {
            using (var response = await SendRequest(new Uri("https://imdb8.p.rapidapi.com/title/get-full-credits?tconst=" + id)))
            {
                response.EnsureSuccessStatusCode();
                JObject obj = (JObject)JsonConvert.DeserializeObject(await response.Content.ReadAsStringAsync());
                List<string> actors = new List<string>();
                List<string> directors = new List<string>();
                List<string> producers = new List<string>();
                int i = 0;
                foreach (var actor in obj["cast"])
                {
                    if (i == 5)
                        break;
                    var actorid = actor["id"].ToString().Split(new char[] { '/' })[2];
                    using (var resp = await SendRequest(new Uri("https://imdb8.p.rapidapi.com/actors/get-bio?nconst=" + actorid)))
                    {
                        resp.EnsureSuccessStatusCode();
                        JObject obj1 = (JObject)JsonConvert.DeserializeObject(await resp.Content.ReadAsStringAsync());
                        if (context.Actors.Any(x => x.ImdbId == actorid))
                            Actors.Add(context.Actors.FirstOrDefault(x => x.ImdbId == actorid));
                        else
                        {
                            if (obj1["image"] != null)
                                context.Actors.Add(new Actor() { Name = obj1["name"].ToString(),
                                                                Image = obj1["image"]["url"].ToString(),
                                                                ImdbId = actorid });
                            else
                                context.Actors.Add(new Actor() { Name = obj1["name"].ToString(), 
                                                                 Image = "data:image/jpeg;base64,",
                                                                 ImdbId = actorid});
                            context.SaveChanges();
                            Actors.Add(context.Actors.FirstOrDefault(x => x.ImdbId == actorid));
                        }
                    }
                    i++;
                }
                i = 0;
                foreach (var director in obj["crew"]["director"])
                {
                    if (i == 2)
                        break;
                    string directorid = director["id"].ToString().Split(new char[] { '/' })[2];
                    if (context.Directors.Any(x => x.ImdbId == directorid))
                        Directors.Add(context.Directors.FirstOrDefault(x => x.ImdbId == directorid));
                    else
                    {
                        context.Directors.Add(new Director() { Name = director["name"].ToString(),
                                                               ImdbId = directorid });
                        context.SaveChanges();
                        Directors.Add(context.Directors.FirstOrDefault(x => x.ImdbId == directorid));
                    }
                    i++;
                }
                i = 0;
                foreach (var producer in obj["crew"]["producer"])
                {
                    if (i == 2)
                        break;
                    string producerid = producer["id"].ToString().Split(new char[] { '/' })[2];
                    using (var resp = await SendRequest(new Uri("https://imdb8.p.rapidapi.com/actors/get-bio?nconst=" + producerid)))
                    {
                        resp.EnsureSuccessStatusCode();
                        JObject obj1 = (JObject)JsonConvert.DeserializeObject(await resp.Content.ReadAsStringAsync());
                        if (context.Producers.Any(x => x.ImdbId == producerid))
                            Producers.Add(context.Producers.FirstOrDefault(x => x.ImdbId == producerid));
                        else
                        {
                            if (obj1["image"] != null)
                                context.Producers.Add(new Producer() { Name = obj1["name"].ToString(), 
                                                                       Image = obj1["image"]["url"].ToString(),
                                                                       ImdbId = producerid });
                            else
                                context.Producers.Add(new Producer() { Name = obj1["name"].ToString(), 
                                                                       Image = "data:image/jpeg;base64,",
                                                                       ImdbId = producerid });
                            context.SaveChanges();
                            Producers.Add(context.Producers.FirstOrDefault(x => x.ImdbId == producerid));
                        }
                    }
                    i++;
                }

            }
        }
        void Clear()
        {
            this.Genres = new List<Genre>();
            this.Actors = new List<Actor>();
            this.Directors = new List<Director>();
            this.Producers = new List<Producer>();
            Film = new Film();
        }
        public async Task<HttpResponseMessage> SendRequest(Uri uri)
        {            
            var request = new HttpRequestMessage
            {
                Method = HttpMethod.Get,
                RequestUri = uri,
                Headers =
                    {
                        { "x-rapidapi-host", "imdb8.p.rapidapi.com" },
                        { "x-rapidapi-key", key },
                    },
            };
            return await client.SendAsync(request);
        }
    }
}
